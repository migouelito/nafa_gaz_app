...
class CheckoutView extends GetView<CheckoutController> {
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            _buildTypeRow(index, "vente", "Vente", Colors.blue),
            _buildTypeRow(index, "recharge", "Recharge", Colors.green),
            _buildTypeRow(index, "echange", "Échange", Colors.orange),
            const SizedBox(height: 10),
          ],
        ),
      );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Obx(() {
                      var selected = controller.addresses.firstWhere(
                        (a) => a['id'].toString() == controller.selectedAddressId.value,
                        orElse: () => null,
                      );
                      return Text(selected != null ? selected['name'] : "Choisir une adresse", 
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 20),
            const Text("Mes adresses enregistrées", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 17)),
            const SizedBox(height: 15),
              ...
              : Flexible(
                  child: ListView(
                    ...
                    children: controller.addresses.map((addr) {
                      bool isSelected = controller.selectedAddressId.value == addr['id'].toString();
                      return ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: Icon(Icons.location_on_rounded, color: isSelected ? AppColors.generalColor : Colors.grey),
                        title: Text(addr['name'], style: TextStyle(fontWeight: isSelected ? FontWeight.w900 : FontWeight.bold)),
                        subtitle: Text(addr['address'] ?? "", style: const TextStyle(fontSize: 12)),
                        trailing: isSelected ? Icon(Icons.check_circle, color: AppColors.generalColor) : null,
                        onTap: () {
                          controller.selectedAddressId.value = addr['id'].toString();
                          Get.back();
                        },
                      );
                    }).toList(),
  ...
  Widget _buildTypeRow(int index, String type, String label, Color color) {
    var p = controller.selectedProducts[index];
    return Padding(
  ...
  Widget _buildBottomAction(){
    return Container(
      ...
      child: ElevatedButton(
        onPressed: () async{
        bool isSuccess = await controller.validerCommande();
        if(isSuccess) Canpop.popMultiple(3);
      },
